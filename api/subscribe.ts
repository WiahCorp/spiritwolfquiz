...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Step A: Attempt to upsert profile properties in a safe, non-blocking way
    try {
      console.log("[Klaviyo Endpoint] Attempting to upsert profile properties for email:", email);
      const upsertPayload = {
        data: {
          type: "profile",
          attributes: {
      ...
      console.log("[Klaviyo Endpoint] Profile upsert payload:", JSON.stringify(upsertPayload, null, 2));

      const upsertResponse = await fetch("https://a.klaviyo.com/api/profile-upserts", {
        method: "POST",
        headers: {
      ...
      console.log("[Klaviyo Endpoint] Profile upsert status:", upsertResponse.status);
      const upsertData = await upsertResponse.json().catch(() => ({}));
      console.log("[Klaviyo Endpoint] Profile upsert response:", JSON.stringify(upsertData, null, 2));
    } catch (upsertError) {
      console.error("[Klaviyo Endpoint] Profile upsert properties tracking failed (non-blocking):", upsertError);
    }

    // Step B & 4: Send to Klaviyo Subscription API with consent SUBSCRIBED (clean, valid payload ONLY)
    const bodyPayload = {
      data: {
