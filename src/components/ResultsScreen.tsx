...
export default function ResultsScreen({ archetype, userEmail, onReset }: ResultsScreenProps) {
  ...
  const shareText = `🐺 My Spirit Wolf is the ${archetype.name} (${archetype.title})! I am connected to real ${/8|907/i.test(archetype.realWolf.name) ? "" : "Mexican gray "}${archetype.realWolf.name}. Discover yours at https://quiz.hellowiah.com`;
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: shareText,
          url: "https://quiz.hellowiah.com",
        });
      } else {
        await navigator.clipboard.writeText("https://quiz.hellowiah.com");
        triggerFeedback();
      }
    } catch (err) {
      ...
      try {
        ...
        setFeedback(`Please copy this manually:\n\n🐺 My Spirit Wolf is the ${archetype.name} (${archetype.title})!\n\n${archetype.description}`);
      }
    }
  };
          ...
          <div className="relative h-64 sm:h-72 w-full overflow-hidden group">
            ...
            <div className="absolute bottom-4 left-6">
              <span className="text-xs uppercase font-mono text-[#f9d440] tracking-widest font-bold">
                {archetype.id === "A" || archetype.id === "C" ? "Real Wolf in Yellowstone" : "Real Wolf in Mount Taylor"}
              </span>
        ...
        >
          ...
          <div>
            ...
            <div className="text-zinc-300 font-sans text-sm leading-relaxed flex flex-col gap-4">
              ...
              <p>
                At WIAH, we create compostable products from corn and support efforts to restore wolves: not just to protect them, but to help bring them home to the wild where they belong.
              </p>
            </div>
          </div>
        ...
        <AnimatePresence>
          {feedback && (
            ...
            >
              ...
              <span className="text-xs font-sans tracking-wide leading-relaxed whitespace-pre-wrap">{feedback}</span>
            </motion.div>
          )}
        </AnimatePresence>
